...
class OVSegDEMO(MaskFormer):
    ...
    def forward(self, batched_inputs, adapter_return=False):
        """
        Args:
            batched_inputs: a list, batched outputs of :class:`DatasetMapper`.
                Each item in the list contains the inputs for one image.
        ...
        text_features = self.clip_adapter.get_text_features(class_names)
        clip_features = self.clip_adapter.normalize_feature(outputs["pred_logits"])
        ...
        for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
            ...
            # r = [class, h, w]
            # clip_features = [1, nMask, C]
            seg, feat = self.demo_features(clip_features[iter], mask_cls_result, mask_pred_result, image, class_names)
            iter+=1
            height = input_per_image.get("height", image_size[0])
            width = input_per_image.get("width", image_size[1])
            seg = sem_seg_postprocess(seg, image_size, height, width)
            feat = sem_seg_postprocess(feat, image_size, height, width)
            processed_results.append({"sem_seg": seg, "feat_seg": feat})

        if adapter_return:
            return processed_results, self.clip_adapter
        else:
            return processed_results

    def demo_features(self, mask_features, mask_cls, mask_pred, image, class_names):
        # Instead of class label, we want to achieve CLIP features of selected classes
        mask_cls = F.softmax(mask_cls, dim=-1)[..., :-1]
        mask_pred = mask_pred.sigmoid()
        ...
        if self.clip_ensemble:
            ...
            if self.clip_ensemble_weight > 0:
                ...
                mask_cls = clip_cls
                mask_pred = mask_pred[valid_flag]
                mask_features = mask_features[valid_flag]

        bin_mask = mask_pred > self.clip_adapter.mask_thr
        select_cls = torch.zeros(sum(valid_flag), mask_cls.shape[-1], device=self.device)
        select_features = torch.zeros(sum(valid_flag), mask_features.shape[1], device=self.device)
        ...
        for idx in select_mask:
            ...
            select_features[idx] = mask_features[idx]
        semseg = torch.einsum("qc,qhw->chw", select_cls, bin_mask.float())
        semfeat = torch.einsum("qc,qhw->chw", select_features, bin_mask.float())    
        # finalfeatures = torch.zeros((mask_features.shape[1],image.shape[1],image.shape[2]))
        # m, r, c = torch.where(bin_mask==True)
        # finalfeatures[m,r,c]

        # Semantic segmentation, CLIP features, CLIP Adapter
        return semseg, semfeat
